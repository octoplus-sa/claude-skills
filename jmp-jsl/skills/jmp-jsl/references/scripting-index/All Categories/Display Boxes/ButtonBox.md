...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/ButtonBox.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/ButtonBox.html)*

---

# [ButtonBox](#buttonbox)[](#buttonbox "Click to copy url")
...
### [Click](#click)[](#click "Click to copy url")

**Syntax:** obj \<\< Click

**Description:** Simulates a click on the button, running any callbacks if appropriate. A toggle button will change state prior to the callback being invoked. A menu button requires an argument to specify which item to click.

#### [Menu](#menu)[](#menu "Click to copy url")

``` jsl
New Window( "Example",
    ex = Button Box( "State",
        actiontext << Set Text( Char( ex << Get Menu Choice() ) ),
        <<Set Menu Items( {"Start", "Stop"} )
    ),
    H List Box( Text Box( "Last action: " ), actiontext = Text Box( "" ) )
);
Wait( 1 );
ex << Click( "Stop" );
```
...
``` jsl
New Window( "Example", ex = Button Box( "Start", ex << Set Button Name( "Stop" ) ) );
Wait( 1 );
ex << Click();
```

#### [Toggle](#toggle)[](#toggle "Click to copy url")

``` jsl
New Window( "Example",
    ex = Button Box( "Run",
        actiontext << Set Text( If( ex << Get(), "Start", "Stop" ) ),
        <<Style( "Toggle" )
    ),
    H List Box( Text Box( "Last action: " ), actiontext = Text Box( "" ) )
);
...
**Syntax:** pressed = obj \<\< Get

**Description:** Returns 1 if the toggle button is pressed, 0 if not pressed. Only applies to buttons which have Style("Toggle").

**JMP Version Added:** 14

``` jsl
New Window( "Toggle Button Example",
    ex = Button Box( "Toggle",
        If( ex << Get,
...
### [Get Button Name](#get-button-name)[](#get-button-name "Click to copy url")

**Syntax:** name = obj \<\< Get Button Name

**Description:** Returns the button name.

``` jsl
New Window( "Example", ex = Button Box( "Start" ) );
ex << Get Button Name();
```
...
**Syntax:** icon = obj \<\< Get Icon

**Description:** Returns the icon for the button.
...
### [Get Icon Location](#get-icon-location)[](#get-icon-location "Click to copy url")

**Syntax:** left\|right = obj \<\< Get Icon Location

**Description:** Returns the location of an icon in a Button Box. Icon can be displayed to the left or right side of the button text.

``` jsl
New Window( "Example", ex = Button Box( "Pi" ) );
ex << Set Icon( "$SAMPLE_IMAGES/pi.gif" );
ex << Get Icon Location();
```

### [Get Menu Choice](#get-menu-choice)[](#get-menu-choice "Click to copy url")

**Syntax:** obj \<\< Get Menu Choice

**Description:** Gets the index of the last user menu selection

``` jsl
New Window( "Example", b = Button Box( "Popup Menu", Print( b << Get Menu Choice ) ) );
b << set menu items( {"a", "b", "c"} );
```
...
New Window( "Example", b = Button Box( "Popup Menu", Print( b << Get Menu Choice ) ) );
b << set menu items( {"a", "b", "c"} );
b << set menu item state( 1, -1 );
b << get menu item state( 1 );
```

### [Get Menu Items](#get-menu-items)[](#get-menu-items "Click to copy url")

**Syntax:** list of menu items = obj \<\< Get Menu Items

**Description:** Gets the menu items used for popup menu when the button is clicked
...
**Description:** Returns the current style of the button.

**JMP Version Added:** 14

``` jsl
New Window( "Example", ex = Button Box( "Click Me" ) );
ex << Style( "Toggle" );
Print( ex << Get Style() );
```

### [Get Underline Style](#get-underline-style)[](#get-underline-style "Click to copy url")

**Syntax:** style = obj \<\< Get Underline Style

**Description:** Returns the current underline style of the button.

``` jsl
New Window( "Example", ex = Button Box( "Start", ex << Set Button Name( "Stop" ) ) );
Print( ex << Get Underline Style() );
```
...
### [Open Next Outline](#open-next-outline)[](#open-next-outline "Click to copy url")

**Syntax:** obj \<\< Open Next Outline

**Description:** Closes the containing outline box and opens the next one.

``` jsl
New Window( "Example",
    Outline Box( "First", ex = Button Box( "Next", ex << Open Next Outline ) ),
    Outline Box( "Second", Text Box( "Last" ), <<Close )
);
```

### [Restore Focus After Click](#restore-focus-after-click)[](#restore-focus-after-click "Click to copy url")

**Syntax:** obj \<\< Restore Focus After Click( state=0\|1 )

**Description:** Sets the button to preserve the edit focus after being clicked. The default on Windows is to take the focus. The default on Macintosh is to preserve the focus.

``` jsl
New Window( "Example",
    Script Box( "" ),
    ex = Button Box( "Leave Focus", 0 ),
    Button Box( "Take Focus", 0 )
);
ex << Restore Focus After Click( 1 );
```

### [Set](#set)[](#set "Click to copy url")

**Syntax:** obj \<\< Set( pressed, \<run script(0\|1)\> )

**Description:** Set the toggle state of a toggle button to pressed (1) or not pressed (0). Only applies to buttons which have Style("Toggle").

**JMP Version Added:** 14

``` jsl
New Window( "Toggle Button Example", ex = Button Box( "Toggle" ) );
ex << Style( "Toggle" );
ex << Set( 1 );
```

### [Set Button Name](#set-button-name)[](#set-button-name "Click to copy url")

**Syntax:** obj \<\< Set Button Name( name )

**Description:** Sets the button name to the given text.
...
**Syntax:** obj \<\< Set Function( Function( {thisBox}, \<script\> ) \| Method({thisBox}, \<script\>) \| \<symbol\> )

**Description:** Sets the given function for the control. The function's script is run when the button is pressed.

**JMP Version Added:** 14

#### [Class method](#class-method)[](#class-method "Click to copy url")
...
Define Class(
    ...
    _init_ = Method( {},
        New Window( "MyDialog",
            <<Type( "Dialog" ),
            Button Box( "press me", <<SetFunction( clickCallback ) )
        )
    );
    clickCallback = Method( {buttonBox},
        buttonBox << SetButtonName( "thanks" )
    );
);
obj = New Object( "MyDialog" );
```

#### [Inline function](#inline-function)[](#inline-function "Click to copy url")

``` jsl
New Window( "Example",
    Button Box( "press me",
        <<setFunction( Function( {thisBox}, thisBox << setButtonName( "thanks" ) ) )
    )
);
```
...
**Syntax:** obj \<\< Set Icon( icon name\|image filepath )

**Description:** Displays a JMP icon or an image on the button. Use a JMP icon name or the filepath to an image.
...
### [Set Icon Location](#set-icon-location)[](#set-icon-location "Click to copy url")

**Syntax:** obj \<\< Set Icon Location( left,right )

**Description:** Sets the location of the icon to the left or right side of the button text

**Example 1**

``` jsl
New Window( "Example", ex = Button Box( "Go" ) );
ex << Set Icon( "Go" );
ex << Set Icon Location( "left" );
```
...
### [Set Menu Choice](#set-menu-choice)[](#set-menu-choice "Click to copy url")

**Syntax:** obj \<\< Set Menu Choice( index )

**Description:** Sets the index of the user menu selection
...
### [Set Menu Items](#set-menu-items)[](#set-menu-items "Click to copy url")

**Syntax:** obj \<\< Set Menu Items( list of menu items )

**Description:** Sets the menu items to be used for popup menu when the button is clicked
...
**Syntax:** obj \<\< Style( "Normal"\|"Underline"\|"Toggle" )
...
``` jsl
New Window( "Underline Button Example",
    ex = Button Box( "Start", ex << Set Button Name( "Stop" ) )
);
ex << Underline Style( 1 );
```

### [Underline Style](#underline-style)[](#underline-style "Click to copy url")

**Syntax:** obj \<\< Underline Style( state=0\|1 )

**Description:** Sets the button to use underline drawing style instead of standard button style.
...
### [Unhide Next Item](#unhide-next-item)[](#unhide-next-item "Click to copy url")

**Syntax:** obj \<\< Unhide Next Item

**Description:** Reveals hidden items in the containing outline box.

``` jsl
New Window( "Example",
    Outline Box( "Question",
        ex = Button Box( "See Answer", ex << Unhide Next Item ),
        Text Box( "42", <<hide )
    )
);
```
...
[ Previous](BusyLightBox.html "BusyLightBox") [Next ](CalendarBox.html "CalendarBox")

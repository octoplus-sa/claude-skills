...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/CheckBoxBox.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/CheckBoxBox.html)*

---

# [CheckBoxBox](#checkboxbox)[](#checkboxbox "Click to copy url")
...
**Description:** Returns a display box that displays a CheckBox for each item in the inputted list.
...
**Description:** Sets the item at the index to be enabled (1) or disabled (0).
...
**Syntax:** 0\|1 = obj \<\< Get( \<index\> )

**Description:** Returns if the check box is checked (1), unchecked (0), or indeterminate (empty()) for Three State checkboxes.
...
New Window( "Mountains",
    tb = Table Box(
        ...
        cb = Check Box( {"A", "B", "C", "D"} )
    )
);
cb << Set Heading( "Climbed" );
cb << Get Heading;
```
...
New Window( "Example",
    cb = Check Box(
        {"One", "Two", "Three"},
        Print( "Selected: " || Char( cb << Get Selected() ) )
    )
);
Wait( 1 );
...
New Window( "Example",
    Check Box(
        {"a", "b", "c"},
        <<setfunction(/*anonymous function...*/Function(
                {thisBox, iCheck/*optional parameter for check box tells which box changed*/},
                irow << SetText( Char( iCheck ) );
                iname << SetText(
                    (thisBox << GetItems)[iCheck]/*index the list of check box names from <<GetItems*/
                );
                ivalue << SetText(
                    Char(
                        thisBox << Get( iCheck )/*<<Get returns the value of the iCheck box*/
                    )
                );
            )
        )
    ),
    H List Box(/*irow,iname,ivalue are the text boxes that are updated when a check box changes*/
        Text Box( "row: " ),
        irow = Text Box( "" ),
        Text Box( " " ),
        iname = Text Box( "" ),
...
**Description:** Returns a list of strings of checked items.
...
**Syntax:** obj \<\< Get Selected Indices
...
cb << Set( 1, 1 );
cb << Set( 3, 1 );
Print( cb << Get Selected Indices() );
```

### [Get Selected Indices As Matrix](#get-selected-indices-as-matrix)[](#get-selected-indices-as-matrix "Click to copy url")

**Syntax:** indices = obj \<\< Get Selected Indices As Matrix

**Description:** Returns a list of all of the selected indices as a matrix
...
cb << Set( 1, 1 );
cb << Set( 3, 1 );
Print( cb << Get Selected Indices As Matrix() );
```

### [Get Three State](#get-three-state)[](#get-three-state "Click to copy url")

**Syntax:** obj \<\< Get Three State( \<Boolean\> )

**Description:** Creates a three state checkbox. All items are either 0, 1, or empty().
...
cb << Set Three State( 1 );
cb << Set( 2, Empty() );
Print( cb << Get Three State() );
Print( cb << Get( 2 ) );
```
...
**Description:** Returns if the item at the index is enabled (1) or disabled (0).
...
**Syntax:** obj \<\< Set( \<index\>,\<Boolean\|empty()\>,\<Run Script(0\|1)\> )

**Description:** Sets the check box to be checked (1) or unchecked (0). Using the RunScript option also requires specifying the Boolean argument, as the arguments are positional.

``` jsl
New Window( "Example",
    H List Box(
        cb = Check Box( {"One", "Two", "Three"} ),
        cb2 = Check Box( {"Option 1", "Option 2", "Option 3"} )
    )
);
cb << Set( 3, 1 );
cb2 << Set Three State( 1 );
cb2 << Set( 3, Empty() );
```

### [Set All](#set-all)[](#set-all "Click to copy url")

**Syntax:** obj \<\< Set All( \<Boolean\|empty()\>,\<Run Script(0\|1)\> )

**Description:** Sets all the check boxes to be checked (1) or unchecked (0). Using the RunScript option also requires specifying the Boolean argument, as the arguments are positional.

``` jsl
New Window( "Example", cb = Check Box( {"One", "Two", "Three"} ) );
cb << setFunction( Function( {this, button}, Show( button, this << get( button ) ) ) );
cb << Set All( 1, run script( 1 ) );
```
...
**Description:** Sets the given function for the control. The function's script runs when a check box is clicked.
    ...
    cb = Check Box(
        ...
        <<SetFunction(
            Function( {thisBox, index},
                (thisBox << sib) << settext( "Clicked: " || Char( index ) );
                ((thisBox << sib) << sib) << settext(
                    "Selected: " || Char( thisBox << Get Selected )
                );
            )
        )
    ),
...
### [Set Three State](#set-three-state)[](#set-three-state "Click to copy url")

**Syntax:** obj \<\< Set Three State( \<Boolean\> )
...
states = {1, 1, 1};
cbstates = {0, Empty(), 1};
// clicking on a three state checkbox cycles between checked and unchecked
// indeterminate must be set explicitly
New Window( "Example",
    Check Box(
        {"One", "Two", "Three"},
        <<Set Three State( 1 ),
        <<SetFunction(
            Function( {thisBox, index},
                states[index] = Mod( states[index], 3 ) + 1;
                thisBox << set( index, cbstates[states[index]] );
            )
        )
    )
);
...
[ Previous](CenterBox.html "CenterBox") [Next ](ColBox.html "ColBox")

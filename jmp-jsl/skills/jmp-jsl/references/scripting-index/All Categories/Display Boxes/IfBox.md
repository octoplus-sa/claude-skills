...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/IfBox.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/IfBox.html)*

---

# [IfBox](#ifbox)[](#ifbox "Click to copy url")
...
**Description:** Returns the state of the IfBox.

``` jsl
New Window( "Example",
    Border Box( top( 9 ), bottom( 9 ), Left( 50 ),
        Button Box( "switch", /* this button controls two mutually exclusive If Box displays */
            diagBox << set( !(diagBox << get()) );/* use the opposite of the old value */
            surfBox << set( !(surfBox << get()) );
        )
    ),
    H List Box(
        surfBox = If Box(
            0, /* this one is OFF at first */SurfacePlot(
                ControlPanel( 0 ),
                Frame3D( SetGraphSize( 200, 200 ), SetSpin( -0.01, 0, 0.03, 0.1 ), )
            )
        ),
        diagBox = If Box(
            1, /* this one is ON at first */Diagram(
                hierarchy(
                    Hier Box(
                        Text Edit Box( "Yield" ),
                        Hier Box( Text Edit Box( "Pressure" ), <<direction( 1 ) ),
                        Hier Box( Text Edit Box( "Temperature" ) )
                    )
                )
            )
        )
...
**Description:** Shows or hides the display boxes within the IfBox.

``` jsl
orangeStatus = 1;
greenStatus = 0;
orangeSpot = Spacer Box( size( 9, 9 ), color( "orange" ) );
greenSpot = Spacer Box( size( 9, 9 ), color( "green" ) );
New Window( "Example",
    orangeIf = If Box( orangeStatus, orangeSpot ),
    greenIf = If Box( greenStatus, greenSpot ),
    Button Box( "orange",
        orangeStatus = !orangeStatus;
        orangeIf << set( orangeStatus );
    ),
    Button Box( "green",
        greenStatus = !greenStatus;
        greenIf << set( greenStatus );
    )
);
```
...
[ Previous](IconStringColBox.html "IconStringColBox") [Next ](IfSeg.html "IfSeg")

...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/MatrixBox.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/MatrixBox.html)*

---

# [MatrixBox](#matrixbox)[](#matrixbox "Click to copy url")
...
**Description:** Returns a display box that contains a matrix.

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
```
...
``` jsl
New Window( "Example", mb = Matrix Box( [1.2, 3.14, 45.6789] ) );
mb << Align Decimal Separator( 1 );
```

### [Column Names](#column-names)[](#column-names "Click to copy url")

**Syntax:** obj \<\< Column Names( strings or list of strings )

**JMP Version Added:** 15

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Column Names( "A", "B", "C" );
```
...
**Description:** Controls whether semantic formats are applied to the matrix. Semantic formats can also be controlled via preferences.

**JMP Version Added:** 17

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Enable Semantic Formats( 0 );
Show( mb << Get Enable Semantic Formats );
```

### [Get](#get)[](#get "Click to copy url")

**Syntax:** matrix = obj \<\< Get

**Description:** Returns a list representing the matrix.
...
**Description:** Returns 1 if the columns align values on the decimal separator, 0 otherwise.

**JMP Version Added:** 17

``` jsl
New Window( "Example", mb = Matrix Box( [1.2, 3.14, 45.6789] ) );
mb << Get Align Decimal Separator;
```

### [Get Effective Format](#get-effective-format)[](#get-effective-format "Click to copy url")

**Syntax:** obj \<\< Get Effective Format( col, \<row\> )

**Description:** Returns a column's effective format, that is the format after applying any matching semantic format. If row is given, returns the effective format for just that row.

**JMP Version Added:** 17

``` jsl
New Window( "Example MB",
    mb = Matrix Box(
        [0.123 0.123, 3.1415 3.1415, 45.6789 45.6789],
        Column Names( "Column1", "Column2" )
    )
);
mb << Set Format( "Fixed Dec", 10, 2 );

// Create a semantic format
Preferences(
    Semantic formatting(
        Add Semantic Format(
            Format Name( "Example MB Format" ),
            Semantic Format( Format( "Scientific", 20, 2 ) ),
            Criteria( Object Name( "Column1" ), Platform Name( "Example MB" ) )
        )
    )
);

Show( mb << Get Effective Format( 1 ), mb << Get Effective Format( 2 ), mb << Get Format );
```
...
**Description:** Get the matrix's Enable Semantic Formats setting. Semantic formats can also be controlled via preferences.
...
**Syntax:** obj \<\< Get Format( width, decimalplaces )

**Description:** Returns the current format specification.
...
**Syntax:** obj \<\< Make Combined Data Table( \<Invisible(bool)\> )

**Description:** Makes a data table from all similar matrices.

**Example 1**

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Make Combined Data Table;
```
...
``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Make Combined Data Table( invisible( 1 ) );
```
...
**Description:** Creates a new data table that contains the values in the MatrixBox.

**Example 1**

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Make into Data Table;
```
...
``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Make into Data Table( invisible( 1 ) );
```

### [Open](#open)[](#open "Click to copy url")

**Syntax:** obj \<\< Open( state=0\|1 )

**Description:** Shows or hides the values within the MatrixBox.
...
### [Row Names](#row-names)[](#row-names "Click to copy url")

**Syntax:** obj \<\< Row Names( strings or list of strings )

**JMP Version Added:** 15

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Row Names( "First", "Second" );
```

### [Set](#set)[](#set "Click to copy url")

**Syntax:** obj \<\< Set( matrix )

**Description:** Set the matrix from a list.

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
Wait( 2 );
mb << Set( [11 22, 33 44, 55 66] );
```

### [Set Conditional Format](#set-conditional-format)[](#set-conditional-format "Click to copy url")

**Syntax:** obj \<\< Set Conditional Format( None\|Correlation\|PValue\|Factor Pattern )

**Description:** Sets the data format of the matrix values.

``` jsl
Open( "$SAMPLE_DATA/Body Measurements.jmp" );
New Window( "Example",
    multi = Multivariate(
        Y(
...
multir = multi << report;
mb = multir[Matrix Box( 1 )];
mb << Set Conditional Format( "Factor Pattern" );
```
...
``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Set Format( "Fixed", 5, 1 );
```

#### [Scientific](#scientific)[](#scientific "Click to copy url")

``` jsl
New Window( "Example", mb = Matrix Box( [1 2 31, 4 3 21] ) );
mb << Set Format( {"Scientific", 8, 2} );
```

### [Sort](#sort)[](#sort "Click to copy url")

**Syntax:** obj \<\< Sort( column, ascending )

**Description:** Sorts the rows of the matrix based on one of the columns. If 0 is specified for the column, the sort is removed.
...
[ Previous](MarkerSeg.html "MarkerSeg") [Next ](MosaicSeg.html "MosaicSeg")

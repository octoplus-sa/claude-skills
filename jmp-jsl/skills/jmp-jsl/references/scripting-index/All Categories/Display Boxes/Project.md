...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/Project.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/Project.html)*

---

# [Project](#project)[](#project "Click to copy url")

## [Item Messages](#item-messages)[](#item-messages "Click to copy url")

### [Add Bookmarks](#add-bookmarks)[](#add-bookmarks "Click to copy url")

**Syntax:** project \<\< Add Bookmarks( {\<File(path)\>, \<Folder( path, \<Expanded(boolean)\> )\>, \<Group(name, \<Expanded(boolean)\>, {contents}\>} )

**Description:** Adds a list bookmarks to the project. Existing bookmarks are retained. The argument is a list of bookmark items, each of which is specified using File(), Folder(), or Group(). The Group() type accepts Files, Folders, and Groups as children using the same syntax recursively.
...
project << Add Bookmarks( {File( "$SAMPLE_DATA/Big Class Families.jmp" )} );
```

### [Contents](#contents)[](#contents "Click to copy url")

**Syntax:** project \<\< Contents()

**Description:** Lists all of the files within a project
...
**Syntax:** project \<\< Extract("file")

**Description:** Extract a specific file from the project. The JMP Open() command will not extract files from the project which are not directly openable by JMP. One example would be parquet files.
...
project = Open( "$SAMPLE_PROJECTS/Big Class.jmpprj" );
project << Extract( "myFile.data" );
```

### [Extract All](#extract-all)[](#extract-all "Click to copy url")

**Syntax:** project \<\< Extract All()

**Description:** Extracts all files from the project into the temporary directory.
...
### [Get Bookmarks](#get-bookmarks)[](#get-bookmarks "Click to copy url")

**Syntax:** project \<\< Get Bookmarks()

**Description:** Returns the list of bookmarks in the project. The format of the returned list is the same as is used by Add Bookmarks() and Set Bookmarks().
...
Print( project << Get Bookmarks() );
```

### [Get Layout](#get-layout)[](#get-layout "Click to copy url")

**Syntax:** project \<\< Get Layout()

**Description:** Returns the window layout of the project. The format is the same as is used by Set Layout().
...
Print( project << Get Layout() );
```

**Example 2**

``` jsl
project = Open( "$SAMPLE_PROJECTS/Pizza.jmpprj" );

Print( project << Get Layout() );
```

### [Get Temp Dir](#get-temp-dir)[](#get-temp-dir "Click to copy url")

**Syntax:** project \<\< Get Temp Dir()

**Description:** Get the path on disk to the project's temporary directory.
...
project = Open( "$SAMPLE_PROJECTS/Big Class.jmpprj" );
path = project << Get Temp Dir();
```

### [Is Extracted](#is-extracted)[](#is-extracted "Click to copy url")

**Syntax:** project \<\< Is Extracted("file")

**Description:** Checks to see if a specific file in the project has been extracted to the temporary directory. The JMP Open() command will not extract files from the project which are not directly openable by JMP. One example would be parquet files.
...
project = Open( "$SAMPLE_PROJECTS/Big Class.jmpprj" );
project << Is Extracted( "myFile.data" );
```

### [Project File Path](#project-file-path)[](#project-file-path "Click to copy url")

**Syntax:** project \<\< Project File Path()

**Description:** Get the path on disk of a saved project file
...
project << Run Script( This Project() << project file path() );
```

### [Reset Layout](#reset-layout)[](#reset-layout "Click to copy url")

**Syntax:** project \<\< Reset Layout()

**Description:** Sets the project to use the default window layout.
...
// 2. Open some files
project << Run Script(
    Open( "$SAMPLE_DATA/Big Class.jmp", Set Window ID( "dt" ) );
    New Window( "Big Class - Bivariate of weight by height",
        Set Window ID( "report" ),
        Bivariate( Y( :weight ), X( :height ), )
    );
);

// 3. Set a custom layout
Wait( 3 );
project << Set Layout(
    H Splitter Box(
        <<Set Sizes( {0.85, 0.15} ),
        V Splitter Box(
            <<Set Sizes( {0.5, 0.5} ),
            Tab Page Box( Window ID( "report" ) ),
            Tab Page Box( Window ID( "dt" ) )
        ),
        Tab Page Box( Window ID( "Window List" ) ), 

    )
);

// 4. Go back to the default layout
Wait( 3 );
project << Reset Layout();
```

### [Run Script](#run-script)[](#run-script "Click to copy url")

**Syntax:** project \<\< Run Script(script)

**Description:** Runs a script in the context of the project. Particularly, any windows opened by the script will become part of the project. The script will access JSL names in the project's context, not that of the enclosing script.
...
dt = project << Run Script( Open( "$SAMPLE_DATA/Big Class.jmp" ) );
...
**Syntax:** project \<\< Save Project(\<path\>)

**Description:** Saves the project. A path and file name can be specified to save the project to a specific location; if the project has already been saved, this argument is optional.

Unsaved documents in the project must be saved before the saving the project itself.
...
project << Save As( "$TEMP/my project.jmpprj" );


project << Run Script( dt << Run Script( "Distribution" ) );
...
### [Set Bookmarks](#set-bookmarks)[](#set-bookmarks "Click to copy url")

**Syntax:** project \<\< Set Bookmarks( {\<File(path)\>, \<Folder( path, \<Expanded(boolean)\> )\>, \<Group(name, \<Expanded(boolean)\>, {contents}\>} )

**Description:** Sets the bookmarks for the project. Existing bookmarks are removed. The argument is a list of bookmark items, each of which is specified using File(), Folder(), or Group(). The Group() type accepts Files, Folders, and Groups as children using the same syntax recursively.
...
project << Set Bookmarks(
    {File( "$SAMPLE_DATA/Big Class.jmp" ), File( "$SAMPLE_DATA/not found.jmp" ),
    Folder( "$SAMPLE_DATA/Variability Data" ), Group(
        "Sample Scripts",
        Expanded( 1 ),
        {File( "$SAMPLE_SCRIPTS/demoCorr.jsl" ), Group(
            "More Sample Scripts",
            {File( "$SAMPLE_SCRIPTS/Scene3D/Teapot.jsl" )}
        )}
    )}
);
```
...
**Syntax:** obj \<\< Set Layout

**Description:** Sets the window layout of the project.
...
### [Show Bookmarks](#show-bookmarks)[](#show-bookmarks "Click to copy url")

**Syntax:** project \<\< Show Bookmarks(boolean)

**Description:** Shows or hides the Bookmarks pane in the project
...
project = Open( "$SAMPLE_PROJECTS/Big Class.jmpprj" );
Wait( 2 );
project << Show Bookmarks( 0 );
Wait( 2 );
project << Show Bookmarks( 1 );
```

### [Show Contents](#show-contents)[](#show-contents "Click to copy url")

**Syntax:** project \<\< Show Contents(boolean)

**Description:** Shows or hides the Contents pane in the project
...
project = Open( "$SAMPLE_PROJECTS/Big Class.jmpprj" );
Wait( 2 );
project << Show Window List( 0 );
Wait( 2 );
project << Show Internal Files( 1 );
```

### [Show Log](#show-log)[](#show-log "Click to copy url")

**Syntax:** project \<\< Show Log(boolean)

**Description:** Shows or hides the Log pane in the project
...
### [Show Recent Files](#show-recent-files)[](#show-recent-files "Click to copy url")

**Syntax:** project \<\< Show Recent Files(boolean)

**Description:** Shows or hides the Recent Files pane in the project
...
project << Show Window List( 0 );
Wait( 2 );
project << Show Recent Files( 1 );
```

### [Show Workspace](#show-workspace)[](#show-workspace "Click to copy url")

**Syntax:** project \<\< Show Workspace(boolean)

**Description:** Shows or hides the Workspace pane in the project
...
[ Previous](PopupBox.html "PopupBox") [Next ](PropertyEditorBox.html "PropertyEditorBox")

...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/RadioBox.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/RadioBox.html)*

---

# [RadioBox](#radiobox)[](#radiobox "Click to copy url")
...
**Description:** Returns a display box to show a list or radio buttons. The Script is run when a radio button is clicked.

``` jsl
New Window( "Example",
    rb = Radio Box( {"True", "False", "None of the above"}, Print( "Selection Changed" ) )
);
```
...
**Description:** Enables or disables an individual item in a radio box

``` jsl
New Window( "Example - disable Green", r = Radio Box( {"Red", "Green", "Blue"} ) );
r << enable item( 2, 0 );
```
...
**Description:** Returns the 1-based index of the selected radio button.

``` jsl
New Window( "Example", r1 = Radio Box( {"Red", "Green", "Blue"}, Print( r1 << Get ) ) );
```
...
New Window( "Mountains",
    tb = Table Box(
        ...
        rb = Radio Box( {"A", "B", "C", "D"} )
    )
);
rb << Set Heading( "Climbed" );
rb << Get Heading;
```
...
**Syntax:** selectedText = obj \<\< Get Selected

**Description:** Gets the text of the selected radio button

``` jsl
New Window( "Example",
    r1 = Radio Box( {"Red", "Green", "Blue"}, Print( r1 << Get Selected ) )
);
```

### [Group](#group)[](#group "Click to copy url")

**Syntax:** obj \<\< Group( radiobox )

**Description:** Groups radio boxes together so that only one radio button in the group is selected at a time

``` jsl
r1 = Radio Box( {"On"}, Print( r1 << Get ) );
r2 = Radio Box( {"Off"}, Print( r2 << Get ) );
...
**Syntax:** obj \<\< Index Of( string )

**Description:** Returns the index of a string, or 0 if not found

``` jsl
New Window( "Example", r = Radio Box( {"Red", "Green", "Blue"} ) );
r << Index Of( "Green" );
```
...
**Syntax:** obj \<\< Item Enabled( index )

**Description:** Checks to determine whether an individual item in a radio box is enabled or disabled
...
**Description:** Sets the given function for the control. The function's script runs when a radio box is clicked.
...
rb << Set Tips( {"I agree", "I disagree", "No comment"} );
```
...
[ Previous](PropertyEditorBox.html "PropertyEditorBox") [Next ](RangeSeg.html "RangeSeg")

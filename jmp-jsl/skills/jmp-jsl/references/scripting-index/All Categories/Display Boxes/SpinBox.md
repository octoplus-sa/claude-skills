...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/SpinBox.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/SpinBox.html)*

---

# [SpinBox](#spinbox)[](#spinbox "Click to copy url")
...
**Description:** Returns a display box to show a button with up and down controls. The script argument is invoked with an argument indicating the direction of the arrow clicked (negative is down, positive is up). A magnitude of 1 indicates a single click. Larger values might be used to indicate a repeating action.
...
**Syntax:** obj \<\< Click( \<value\> )

**Description:** Simulates a click on the button, running any attached script.
...
**Syntax:** obj \<\< Decrement( \<value\> )

**Description:** Simulates a click on the down arrow, running any attached script.
...
**Syntax:** obj \<\< Increment( \<value\> )

**Description:** Simulates a click on the up arrow, running any attached script.
...
**Syntax:** obj \<\< Set Function( Function( {thisBox, increment}, \<script\> ) )

**Description:** Sets the given function for the control. The function's script runs when the spin box's buttons are pressed. \[increment\] is the increment set for the number box.

**JMP Version Added:** 14

``` jsl
New Window( "Example", Lineup Box( 2, nb = Number Edit Box( 0 ), sb = Spin Box() ) );
nb << Set Increment( 5 );
sb << Set Function(
    Function( {thisBox, increment},
        ((thisBox << parent) << child) << increment( increment )
    )
);
```
...
[ Previous](SpacerBox.html "SpacerBox") [Next ](SplitterBox.html "SplitterBox")

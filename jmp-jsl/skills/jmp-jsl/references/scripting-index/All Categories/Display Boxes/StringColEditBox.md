...
*Source: [https://jsl.jmp.com/All%20Categories/Display%20Boxes/StringColEditBox.html](https://jsl.jmp.com/All%20Categories/Display%20Boxes/StringColEditBox.html)*
...
**Description:** Returns a display box that displays editable strings in a column.

``` jsl
New Window( "Example",
    col = String Col Edit Box(
        "Strings",
...
col << Set Function(
    Function( {this, which},
        Print( "row " || Char( which ) || " changed to " || (col << Get( which )) )
    )
);
col << Get Function;
```
...
col << Set Function(
    Function( {thisBox, which},
        Print( "row " || Char( which ) || " changed to " || (thisBox << Get( which )) )
    )
);
```
...
[ Previous](StringColBox.html "StringColBox") [Next ](TabListBox.html "TabListBox")
